...
import 'dart:io';
import 'package:ElMassaConsult/auth-panel/loginsission.dart';
import 'package:ElMassaConsult/auth-panel/welcome-screen.dart';
import 'package:ElMassaConsult/office_app/1/search_orders_screen.dart';
import 'package:ElMassaConsult/office_app/general/categories_screen.dart';
...
class OfficeGovScreenWindows extends StatefulWidget {
  final String? userName;
  const OfficeGovScreenWindows({super.key, this.userName});

  @override
  State<OfficeGovScreenWindows> createState() => _OfficeGovScreenWindowsState();
}

class _OfficeGovScreenWindowsState extends State<OfficeGovScreenWindows> {
  ...
  void initState() {
    super.initState();
    _loadFirestoreData();
     _sessionId = AppConstant.sissionid;
  }

  Future<void> _loadFirestoreData() async {
    try {
      final [teamWorkCodeResult] = await Future.wait([
        FirebaseFirestore.instance.collection('officeTeamWorkRoles').get(),
      ]);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading
          ...
          : Container(
              color: const Color.fromARGB(255, 3, 31, 88),
              child: ListView(
                children: [
                  _buildHeader(),
 ...
 AppBar _buildAppBar() {
  return AppBar(
    actions: [ IconButton(
          icon: const Icon(Icons.vpn_key), // أيقونة للـ Session ID
          onPressed: () async {
            await _updateSessionId();
            // بعدين تقدر تستخدمه في أي API تاني
            print("${AppConstant.sissionid}");
          },
        ),
        SizedBox(width: 10),
      IconButton(
  ...
  Future<void> _updateSessionId() async {
    try {
      ...
      // Call the fetchSessionId function from loginsission.dart with credentials
      await fetchSessionId(username: 'masa1', password: 'mc#all\$1#');
      
      // Close the loading dialog
      if (mounted) {
        Navigator.of(context, rootNavigator: true).pop();
        // Update the UI with the new session ID
        setState(() {
          _sessionId = AppConstant.sissionid;
        });
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('فشل في تحديث معرف الجلسة: $e')),
        );
      }
    }
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          Text(
            _sessionId ?? AppConstant.sissionid ?? 'غير مدخل',
