...
class NewOrdersScreen extends StatefulWidget {
  ...
  @override
  State<NewOrdersScreen> createState() => _SiteTrackingTabBarScreenState();
}

class _SiteTrackingTabBarScreenState extends State<NewOrdersScreen>
 ...
 final ValueNotifier<bool> _isExporting = ValueNotifier(false);
  ...
  Future<void> _uploadOrders() async {
    ...
    final uploader = OrderUploader(
      selectedGov: widget.govName,
      surveyTeamId: widget.userCode,
    );
    final resultMsg = await uploader.fetchAndUploadAllPages();
    isUploadingNotifier.value = false;

    Get.snackbar(
      "نتيجة اضافة الطلبات",
      resultMsg,
      backgroundColor: resultMsg.startsWith('✅') ? Colors.green : Colors.red,
                  ...
                  builder: (_, isUploading, __) {
                    ...
                    return AnimatedSwitcher(
                      ...
                      child: FloatingActionButton.extended(
                        ...
                        onPressed: isUploading ? null : _uploadOrders,
                        label: Text(isUploading ? 'جارٍ التحديث...' : 'تحديث الطلبات'),
                        icon: isUploading
                            ? const SizedBox(
                                height: 20,
                                width: 20,
                        ...
                        backgroundColor:
                            isUploading ? Colors.grey : Colors.blueAccent,
                      ),
                    );
                  },
                )
  ...
  try {
    ...
    for (var doc in ordersSnapshot.docs) {
      ...
      sheetObject
          ...
          .value = TextCellValue(data['orderNumber']?.toString() ?? '');
      sheetObject
          .cell(CellIndex.indexByString('B$rowIndex'))
          .value = data['distributionDate'] != null
              ? TextCellValue(data['distributionDate'].toDate().toIso8601String())
              : TextCellValue('');
      sheetObject
          .cell(CellIndex.indexByString('C$rowIndex'))
          .value = data['surveyingDate'] != null
              ? TextCellValue(data['surveyingDate'].toDate().toIso8601String())
              : TextCellValue('');
      sheetObject
          .cell(CellIndex.indexByString('D$rowIndex'))
          .value = TextCellValue(data['name'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('E$rowIndex'))
          .value = TextCellValue(data['phoneNumber'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('F$rowIndex'))
          .value = TextCellValue(data['unitType'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('G$rowIndex'))
          .value = TextCellValue(data['areaM2'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('H$rowIndex'))
          .value = TextCellValue(data['governorate'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('I$rowIndex'))
          .value = TextCellValue(data['departmentOrCenter'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('J$rowIndex'))
          .value = TextCellValue(data['sheikhdomOrVillage'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('K$rowIndex'))
          .value = TextCellValue(data['unitNumber'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('L$rowIndex'))
          .value = TextCellValue(data['streetName'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('M$rowIndex'))
          .value = TextCellValue(data['distinctiveSigns'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('N$rowIndex'))
          .value = TextCellValue(data['team'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('O$rowIndex'))
          .value = TextCellValue(data['orderStatus'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('P$rowIndex'))
          .value = TextCellValue(data['reasonForInability'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('Q$rowIndex'))
          .value = TextCellValue(data['reviewStatus'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('R$rowIndex'))
          .value = TextCellValue(data['companyName'] ?? '');
      sheetObject
          .cell(CellIndex.indexByString('S$rowIndex'))
          .value = TextCellValue(data['engName'] ?? '');
      rowIndex++;
    }
